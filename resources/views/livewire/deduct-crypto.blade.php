<div class="deduct-crypto-page mt-40">
    <div class="max-w-2xl mx-auto">
        ...
        @else
            <div class="bg-gradient-to-br from-indigo-950 to-purple-900 rounded-2xl shadow-lg border border-white/20 p-6">
                ...
                {{-- Deduct crypto to portfolio --}}
                <form wire:submit.prevent="deductCryptoToPortfolio">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {{-- sell price --}}
                        <div>
                            <label for="crypto_sell_price" class="block font-medium text-gray-300">
                                Sell price (USD)
                            </label>
                            <input id="crypto_sell_price" wire:model="crypto_sell_price" type="text"
                                class="p-2 mt-2 w-full rounded-lg bg-gray-700 border border-gray-600 text-white placeholder-gray-400 focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 shadow-sm transition"
                                required>
                            @error('crypto_sell_price')
                                <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        {{-- total received --}}
                        <div>
                            <label for="total_received" class="block font-medium text-gray-300">
                                Total received
                            </label>
                            <input id="total_received" wire:model="total_received" type="text"
                                class="p-2 mt-2 w-full rounded-lg bg-gray-700 border border-gray-600 text-white placeholder-gray-400 focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 shadow-sm transition"
                                required>
